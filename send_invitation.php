  ...
  if (empty($eror_search)) {
    ...
    if ($search_sql->rowCount() > 0) {
      ...
      foreach ($resul as $rows_result) {
        $id_destinataire = $rows_result['id_user'];
...
if(isset($_POST['invitation_form'])){
  echo 'Hello';
  $msg_send = "";
  $id_user_dest = $_POST['id_destinataire'];
  $id_user_emet = $_POST['id_emetteur'];
  ...
  $req_sql->execute(array(
    ':id_emetteur'=>$id_user_emet,
    ':id_destinataire'=>$id_user_dest,
    ':statut'=>'En attente'
  ));
